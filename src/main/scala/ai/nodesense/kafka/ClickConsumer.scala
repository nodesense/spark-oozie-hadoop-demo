...
class ClickConsumer(val brokers: String,
               val groupId: String,
               val topic: String) {
  ...
  def run() = {
    println("click consumer running");
...
object ClickConsumer   {
  ...
  def runConsumer() = {
    ...
    val example = new ClickConsumer(broker, groupId, topic)
    example.run()
  }
